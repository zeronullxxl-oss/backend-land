...
async function sendFBEvent(lead) {
  ...
  const result = await httpsPost('graph.facebook.com', `/v19.0/${pixelId}/events?access_token=${FB_ACCESS_TOKEN}`, payload);
...
app.get('/admin/stats', authMiddleware, async (req, res) => {
  ...
  const days = [];
  for (let i = 6; i >= 0; i--) {
    const dayParams = sees ? [sees, i] : [i];
    const dayWhere  = sees ? `WHERE buyer=$1 AND created_at::date = CURRENT_DATE - $2` : `WHERE created_at::date = CURRENT_DATE - $1`;
    const r = await pool.query(`SELECT COUNT(*) FROM leads ${dayWhere}`, dayParams);
    const d = new Date(); d.setDate(d.getDate() - i);
    days.push({ date: d.toISOString().slice(0,10), count: parseInt(r.rows[0].count) });
  }
