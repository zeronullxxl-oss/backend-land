...
const DEFAULT_BUYERS = {
  'gpttrade':  { password: 'pass123',   name: 'GPTrade',  geo: 'MX' },
  'nepravda':  { password: 'pass456',   name: 'nepravda', geo: 'MX' }
};
...
function authenticateBuyer(req) {
  ...
  if (buyer && buyer.password === password) {
    return { login, name: buyer.name, geo: buyer.geo, isSuper: false };
  }
  return null;
}
...
app.post('/admin/auth', (req, res) => {
  ...
  res.json({ ok: true, token: login+':'+password, name: buyer.name, geo: buyer.geo, isSuper: false });
});
...
app.get('/admin/stats', authMiddleware, (req, res) => {
  ...
  if (!req.buyer.isSuper) {
    leads = leads.filter(l => l.buyer === req.buyer.login || l.buyer === req.buyer.name);
  }
  const now = new Date();
